   ...
   <!-- Page Header -->
    ...
    <div id="content" role="main">
       ...
       <section class="section section-alt">
        <div class="row-fluid">
          <div class="super-hero-unit">
            <figure>
              <img alt="some image" src="images/assets/landscapes/donor.jpg">
      ...
      <section class="section section-padded">
        ...
        if (isset($_POST['areaSearch'])) {
          $area = $_POST["area"];
          $dinfo = "SELECT * FROM donor,address where address.address_id = donor.donor_id and street = '$area'";
          $dresult = $conn->query($dinfo);
        if ($dresult->num_rows > 0) {
    
        while($row = $dresult->fetch_assoc()) {
        echo "<tr>";
        if($row["donor_img_path"]===""){
             echo "<td>
              ...
              echo "<td>
                    <div class=\"round-box box-small\">
                      <a class=\"box-inner\" href=\"#\">
                        <img alt=\"some image\" class=\"img-circle\" src=" . $row["donor_img_path"] .">
                      </a>
                    <td>";
        }
        ...
        echo "<td> " .  $row["donor_bloodgroup"]. "</td>";
    ...
    echo "<tr><td></td><td>0 results</td></tr>";
        ...
        elseif (isset($_POST['bloodgroupSearch'])) {
          ...
          $dinfo = "SELECT * FROM donor,address where address.address_id = donor.donor_id and donor_bloodgroup = '$bloodgroup'";
        $dresult = $conn->query($dinfo);
        if ($dresult->num_rows > 0) {
        ...
        //echo "id: " . $row["donor_id"]. " - Name: " . $row["donor_name"]. "- Gender: " . $row["donor_gender"]. "- Age: " . $row["donor_age"]. "- BloodGroup: " . $row["donor_bloodgroup"]. "- Height: " . $row["donor_height"]. "- Weight: " . $row["donor_weight"]. "- BP: " . $row["donor_bp"]. "- Eligibility: " . $row["donor_eligibility"]. "- Medication: " . $row["donor_medication"]. "<br>";
    ...
    echo "<tr><td></td><td>0 results</td></tr>";
        ...
        $dinfo = "SELECT * FROM donor,address where address.address_id = donor.donor_id";
        $dresult = $conn->query($dinfo);
        if ($dresult->num_rows > 0) {
            // output data of each row
              ...
              <div class="sidebar-widget widget_search">
                <form name="area" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" >
                  ...
                  <h4 class="sidebar-header">Search By Area</h4>
                    <input class="span12" name="area" placeholder="search" type="text" value=<?php echo "".$area.""; ?> >
                ...
                <form name="bloodgroup" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" >
                  ...
                  <h4 class="sidebar-header">Search By BloodGroup</h4>
                    <input class="span12" name="bloodgroup" placeholder="search" type="text" value=<?php echo "".$bloodgroup.""; ?>>
